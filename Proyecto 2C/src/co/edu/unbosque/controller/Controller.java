...
/**
 * Clase controlador que se encarga de realizar todas las acciones y capturar
 * los datos de la vista para implementar los metodos creados en el modelo
 * 
...
public class Controller implements ActionListener {
	 ...
	 */
	public Controller(VentanaPrincipal vp) {
		super();
		this.vp = vp;
		actionListener(this);
		...
		arraypersonas = archivo.leerArchivo();
	}
	 ...
	 * 
	 * @param escuchador Genera una accion si se Utiliza la clase ActionListener
	 *                   despues de un clic
	 */
	...
	/**
	 * Este metodo utiliza los valores de la clase ActionListener para realizar una
	 * accion en caso de un clic o de oprimir un boton <b>pre</b> Si se oprime un
	 * boton debe suceder algo<br>
	 * <b>post</b> Se genero una accion al oprimir un boton<br>
	 * 
	 * @param evento Utiliza las propuedades de la clase ActionEvent en cuanto hay
	 *               un evento o clic en la aplicacion
	 */
	@Override
	public void actionPerformed(ActionEvent event) {
		...
		if (vp.pp.cdp.regresarBtn == event.getSource()) { // Oprimir el regresar dentro del panel consulta/dep ,se
															// devuelve a la ventana principal
			vp.setVisible(true);
		...
		if (vp.pu.cu.guardarBtn == event.getSource()) {
			...
			fechaE = (String) vp.pu.cu.diaExpCmb.getSelectedItem() + "-" + vp.pu.cu.mesExpCmb.getSelectedItem() + "-"
					+ vp.pu.cu.anoExpCmb.getSelectedItem();
			fechaN = (String) vp.pu.cu.diaNacCmb.getSelectedItem() + "-" + vp.pu.cu.mesNacCmb.getSelectedItem() + "-"
					+ vp.pu.cu.anoNacCmb.getSelectedItem();

			if ((String.valueOf(vp.pu.cu.numeroIdentificacionTxt.getText()).isEmpty())
					|| vp.pu.cu.nombreUnoTxt.getText().isEmpty() || vp.pu.cu.nombreDosTxt.getText().isEmpty()
					|| vp.pu.cu.apellidoUnoTxt.getText().isEmpty() || vp.pu.cu.apellidoDosTxt.getText().isEmpty()) {
				...
				Persona persona = new Persona(cedula, fechaE, lugar_exp, nombre1, nombre2, apellido1, apellido2, fechaN,
						lugar_nac, sexo);
		...
		// Boton para ir atras de la ventana agregar usuario(JFrame CreacionUsuario) a
		...
		// Filtra la informacion de las cedulas inscritas y muestra la informacion del
		// ciudadano segun la cedula que se busque
				...
				if (sistema.arraypersonas.isEmpty() == false) {
					for (int i = 0; i <= sistema.arraypersonas.size() - 1; i++) {
						if (num == Integer.parseInt(sistema.arraypersonas.get(i).getNumero_cedula())) {
							nombre = sistema.arraypersonas.get(i).getApellido1() + " "
									+ sistema.arraypersonas.get(i).getApellido2() + " "
									+ sistema.arraypersonas.get(i).getNombre1() + " "
									+ sistema.arraypersonas.get(i).getNombre2() + " C.C. "
			...
			if (sistema.arraypersonas.isEmpty() == false) {
				for (int i = 0; i <= sistema.arraypersonas.size() - 1; i++) {
					if (num2 == Integer.parseInt(sistema.arraypersonas.get(i).getNumero_cedula())) {
						...
						mensaje = sistema.modificarCiudadano(i, vp.pu.mu.nombreUnoTxt.getText(),
								vp.pu.mu.nombreDosTxt.getText(), vp.pu.mu.apellidoUnoTxt.getText(),
								vp.pu.mu.apellidoDosTxt.getText());
						vp.mostrarMensajes(mensaje);
